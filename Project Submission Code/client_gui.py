...
def searchAndDownloadFile(filename, downloadDirectory):
    ...
    key_pair = RSA.generate(2048) # Generate new RSA keys, for asymmetric encryption of the symmetric key (hybrid encryption)
    
    # prepare the public key for sending via the server to the seeder/folder_monitor
    public_key = key_pair.publickey().exportKey() 
    public_key_decoded = public_key.decode('utf-8')

    # prepare the private key for passing as a command line argument to the leecher process
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
        ...
        client_socket.sendall(b'client')  # Identify this client as a 'client'

        # send search request to the server to check if the file exists on the server
        ...
        # if the file exists on the server, ask the user if they want to download it
        if response_data['exists']:
            ...
            # if yes, (True) send a download request to the server
            if result:
                ...
                # if the server responds with a transfer command, start the leecher process
                if transfer_data['type'] == 'TRANSFER':
                    ...
                    # convert the public key of the seeder to base64 encoding, to pass as a command line argument
...
# Function to to launch the folder monitor
